...
OnOffApplication::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::OnOffApplication")
    ...
    .AddAttribute ("Protocol", "The type of protocol to use.",
                   ...
                   MakeTypeIdChecker ())
    .AddAttribute ("BindtoDevice", "Whether bind to a netdevice",
		   UintegerValue (0),
		   MakeUintegerAccessor (&OnOffApplication::m_bindtoDevice),
		   MakeUintegerChecker<uint32_t> (1))
    .AddAttribute ("NetDevice", "Binding to the specific netdevice.",
		   PointerValue(),
		   MakePointerAccessor (&OnOffApplication::m_netdevice),
  ...
  if (!m_socket)
    {
      ...
      if (m_bindtoDevice)
	m_socket->BindToNetDevice(m_netdevice);
