  ...
  if (m_socket == 0)
    {
      ...
      if (Ipv4Address::IsMatchingType(m_peerAddress) == true)
        {
//          m_socket->Bind();
          Ipv4Address ad = Ipv4Address::ConvertFrom(m_peerAddress);

          InetSocketAddress local = InetSocketAddress ( InetSocketAddress (Ipv4Address::GetAny (), m_peerPort));
          m_socket->Bind (local);
          m_socket->BindToNetDevice (this->m_node->GetDevice(0));
          m_socket->Connect (InetSocketAddress (ad, m_peerPort));
        }
...
//  m_socket->Send (p);
  Ipv4Address ad = Ipv4Address::ConvertFrom(m_peerAddress);
  InetSocketAddress ad2 = InetSocketAddress (ad, m_peerPort);
  m_socket->SendTo (p, 0, ad2);
