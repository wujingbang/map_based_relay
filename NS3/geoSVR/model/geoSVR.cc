...
struct DeferredRouteOutputTag: public Tag {
	...
	static TypeId GetTypeId() {
		static TypeId tid =
				TypeId("ns3::geoSVR::DeferredRouteOutputTag").SetParent<Tag>();
		return tid;
	}
...
RoutingProtocol::RoutingProtocol() :
		HelloInterval(Seconds(1)), MaxQueueLen(6400), MaxQueueTime(
				Seconds(300)), m_queue(MaxQueueLen, MaxQueueTime), HelloIntervalTimer(
				Timer::CANCEL_ON_DESTROY), PerimeterMode(false), m_nbFromMbr(
				false), m_start(false), m_queueNum(0) {
...
bool RoutingProtocol::RouteInput(Ptr<const Packet> p, const Ipv4Header &header,
		...
		ErrorCallback ecb) {
	NS_LOG_FUNCTION(
			this << p->GetUid () << header.GetDestination () << idev->GetAddress ());
	...
	if (m_ipv4->IsDestinationAddress(dst, iif)) {
		...
		if (!tHeader.IsValid()) {
			NS_LOG_DEBUG(
					"geoSVR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
			return false;
		}
...
void RoutingProtocol::DeferredRouteOutput(Ptr<const Packet> p,
	...
	if (result) {
		NS_LOG_LOGIC(
				"Add packet " << p->GetUid () << " to queue. Protocol " << (uint16_t) header.GetProtocol ());
...
//
//		p->RemoveHeader(tmphdr);
			DatapacketHeader hdr(length, m, t, c, srcPos.x, srcPos.y, dstPos.x,
					dstPos.y, enpaths);
			p->AddHeader(hdr);
			TypeHeader tHeader(GEOSVRTYPE_DATAPACKET);
			p->AddHeader(tHeader);
			...
			nextHop = m_neighbors.find_furthest_nhop(roadid1, roadid2, roadid3,
					srcPos.x, srcPos.y, nextPos.x, nextPos.y, m_comm_range);
...
//
//		  p->RemoveHeader(tmphdr);
				DatapacketHeader hdr(length, m, t, c, srcPos.x, srcPos.y,
						dstPos.x, dstPos.y, enpaths);
				p->AddHeader(hdr);
				TypeHeader tHeader(GEOSVRTYPE_DATAPACKET);
				p->AddHeader(tHeader);
...
void RoutingProtocol::NotifyInterfaceUp(uint32_t interface) {
	...
	if (l3->GetNAddresses(interface) > 1) {
		NS_LOG_WARN(
				"geoSVR does not work with more then one address per each interface.");
	}
...
void RoutingProtocol::RecvHello(Ptr<Socket> socket) {
	...
	if (l3->GetNAddresses((interface) == 1)) {
		...
		if (!socket) {
			...
			socket->SetRecvCallback(
					MakeCallback(&RoutingProtocol::RecvHello, this));
		...
		NS_LOG_LOGIC(
				"geoSVR does not work with more then one address per each interface. Ignore added address");
	}
}
...
void RoutingProtocol::SendHello() {
	...
	for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
		...
		Ptr<Socket> socket = j->first;
		Ipv4InterfaceAddress iface = j->second;
		HelloHeader helloHeader(x, y, s, h,
				m_ipv4->GetAddress(1, 0).GetLocal());
...
Ptr<Ipv4Route> RoutingProtocol::LoopbackRoute(const Ipv4Header & hdr,
	...
	NS_ASSERT_MSG(rt->GetSource() != Ipv4Address(),
			"Valid geoSVR source address not found");
...
bool RoutingProtocol::Forwarding(Ptr<const Packet> packet,
	...
	if (m_neighbors.isNeighbor(dst, relayPos, m_comm_range)) {
		...
		NS_ASSERT(route != 0);
		NS_LOG_DEBUG(
				"Exist route to " << route->GetDestination () << " from interface " << route->GetOutputDevice ());
		NS_LOG_LOGIC(
				route->GetOutputDevice () << " forwarding to " << dst << " from " << origin << " through " << route->GetGateway () << " packet " << p->GetUid ());
...
//      nextHop = m_neighbors.find_next_hop (roadid1, roadid2, roadid3, relayPos.x, relayPos.y, nextPos.x, nextPos.y);
		nextHop = m_neighbors.find_furthest_nhop(roadid1, roadid2, roadid3,
				relayPos.x, relayPos.y, nextPos.x, nextPos.y, m_comm_range);
		if (nextHop != Ipv4Address::GetZero()) {
			NS_LOG_DEBUG("Destination: " << dst);
...
//              m_map.getRoadByNode(paths[0], paths[1]))
			...
			NS_LOG_DEBUG(
					"srcPos: "<<srcPos.x<<' '<< srcPos.y<<" dstPos: " <<dstPos.x<<' '<<dstPos.y<<" relayPos: " << relayPos.x << relayPos.y);
			m_neighbors.Print();
			NS_LOG_DEBUG(
					origin << "  "<< dst << "  " << m_ipv4->GetAddress (1, 0).GetLocal ());
...
IpL4Protocol::DownTargetCallback RoutingProtocol::GetDownTarget(void) const {
	return m_downTarget;
}
...
//            paths.erase(paths.begin());
	...
	if (nextHop != Ipv4Address::GetZero()) {
		...
		NS_ASSERT(route != 0);
		NS_LOG_DEBUG(
				"Exist route to " << route->GetDestination () << " from interface " << route->GetSource ());
		NS_LOG_DEBUG(
				this <<" nexthop "<< nextHop << " Location: "<<m_locationService->GetPosition(nextHop));
		if (oif != 0 && route->GetOutputDevice() != oif) {
			NS_LOG_DEBUG("Output device doesn't match. Dropped.");
			sockerr = Socket::ERROR_NOROUTETOHOST;
			return Ptr<Ipv4Route>();
