...
private:
  ...
  void AssignIpAddresses (NetDeviceContainer & d,
						  ...
						  NetDeviceContainer & dData,
						  Ipv4InterfaceContainer & adhocTxInterfacesData);
                        ...
                        int routingTables)
{
  ...
  AssignIpAddresses (d, i, dData, iData);
...
RoutingHelper::SetupRoutingProtocol (NodeContainer & c)
{
  ...
  Ipv4StaticRoutingHelper staticrp;
  ...
  switch (m_protocol)
    ...
    case 4:
      ...
      break;
    case 5:
    	m_protocolName = "STATIC";
    	list.Add (staticrp, 100);
    	break;
    default:
								  ...
								  Ipv4InterfaceContainer & adhocTxInterfacesData)
{
  ...
  addressAdhocData.SetBase ("10.2.0.0", "255.255.0.0");
  adhocTxInterfacesData = addressAdhocData.Assign (dData);
}
...
private:
  ...
  NetDeviceContainer m_adhocTxDevicesData;
  Ipv4InterfaceContainer m_adhocTxInterfaces;
  Ipv4InterfaceContainer m_adhocTxInterfacesData;
  uint32_t m_scenario;
...
VanetRoutingExperiment::ProcessOutputs ()
{
  ...
  double bsm_pdr1 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (1);
  double bsm_pdr2 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (2);
  double bsm_pdr3 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (3);
  double bsm_pdr4 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (4);
  double bsm_pdr5 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (5);
  double bsm_pdr6 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (6);
  double bsm_pdr7 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (7);
  double bsm_pdr8 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (8);
  double bsm_pdr9 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (9);
  double bsm_pdr10 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetCumulativeBsmPdr (10);
  ...
  uint32_t cumulativeWaveBsmBytes = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetTxByteCount ();
...
VanetRoutingExperiment::CheckThroughput ()
{
  ...
  double wavePDR = 0.0;
  int wavePktsSent = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetTxPktCount ();
  int wavePktsReceived = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktCount ();
  if (wavePktsSent > 0)
    {
      int wavePktsReceived = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktCount ();
  ...
  int waveExpectedRxPktCount = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetExpectedRxPktCount (1);
  int waveRxPktInRangeCount = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetRxPktInRangeCount (1);
  double wavePDR1_2 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetBsmPdr (1);
  double wavePDR2_2 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetBsmPdr (2);
  double wavePDR3_2 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetBsmPdr (3);
  double wavePDR4_2 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetBsmPdr (4);
  double wavePDR5_2 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetBsmPdr (5);
  double wavePDR6_2 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetBsmPdr (6);
  double wavePDR7_2 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetBsmPdr (7);
  double wavePDR8_2 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetBsmPdr (8);
  double wavePDR9_2 = 0;//m_waveBsmHelper.GetWaveBsmStats ()->GetBsmPdr (9);
  double wavePDR10_2 = 0;// m_waveBsmHelper.GetWaveBsmStats ()->GetBsmPdr (10);
...
VanetRoutingExperiment::SetupAdhocDevices ()
{
  ...
  // wifichannel for beacons (no obstacle)
  YansWifiChannelHelper wifiChannel;
  // wifichannel for user data (with obstacle)
  YansWifiChannelHelper wifiChannelData;
  wifiChannel.SetPropagationDelay ("ns3::ConstantSpeedPropagationDelayModel");
  wifiChannelData.SetPropagationDelay ("ns3::ConstantSpeedPropagationDelayModel");
  if (m_lossModel == 3)
    {
      ...
      wifiChannelData.AddPropagationLoss (m_lossModelName, "Frequency", DoubleValue (freq), "HeightAboveZ", DoubleValue (1.5));
    }
  else
    {
      wifiChannel.AddPropagationLoss (m_lossModelName, "Frequency", DoubleValue (freq));
      wifiChannelData.AddPropagationLoss (m_lossModelName, "Frequency", DoubleValue (freq));
    }
  ...
  if (m_loadBuildings != 0)
    {
      ...
      wifiChannelData.AddPropagationLoss ("ns3::ObstacleShadowingPropagationLossModel");
    }
  else if (m_fading != 0)
    {
      // if no obstacle model, then use Nakagami fading if requested
      wifiChannel.AddPropagationLoss ("ns3::NakagamiPropagationLossModel");
      wifiChannelData.AddPropagationLoss ("ns3::NakagamiPropagationLossModel");
    }

  // the channel
  Ptr<YansWifiChannel> channel = wifiChannel.Create ();
  Ptr<YansWifiChannel> channelData = wifiChannelData.Create ();

  // The below set of helpers will help us to put together the wifi NICs we want
  YansWifiPhyHelper wifiPhy =  YansWifiPhyHelper::Default ();
  YansWifiPhyHelper wifiPhyData =  YansWifiPhyHelper::Default ();
  wifiPhy.SetChannel (channel);
  wifiPhyData.SetChannel (channel);
  // ns-3 supports generate a pcap trace
  wifiPhy.SetPcapDataLinkType (YansWifiPhyHelper::DLT_IEEE802_11);
  wifiPhyData.SetPcapDataLinkType (YansWifiPhyHelper::DLT_IEEE802_11);
  ...
  NqosWaveMacHelper wifi80211pMacData = NqosWaveMacHelper::Default ();
  WaveHelper waveHelper = WaveHelper::Default ();
  Wifi80211pHelper wifi80211p = Wifi80211pHelper::Default ();
  Wifi80211pHelper wifi80211pData = Wifi80211pHelper::Default ();
  if (m_verbose)
    {
      wifi80211p.EnableLogComponents ();      // Turn on all Wifi 802.11p logging
      wifi80211pData.EnableLogComponents ();
  ...
  wifi80211pData.SetRemoteStationManager ("ns3::ConstantRateWifiManager",
                                      "DataMode",StringValue (m_phyMode),
                                      "ControlMode",StringValue (m_phyMode));
  ...
  wifiPhyData.Set ("TxPowerStart",DoubleValue (m_txp));
  wifiPhyData.Set ("TxPowerEnd", DoubleValue (m_txp));
  ...
  if (m_80211mode == 3)
  ...
  else if (m_80211mode == 1)
    {
      ...
      m_adhocTxDevicesData = wifi80211pData.Install (wifiPhyData, wifi80211pMacData, m_adhocTxNodes);
    }
  else
    {
...
// 0 --- 2 --- 1
  ApplicationContainer serverApps = echoServer.Install (m_adhocTxNodes.Get (1));
  ...
  ApplicationContainer clientApps =
    echoClient.Install (m_adhocTxNodes.Get (0));
...
//                           MilliSeconds (m_txMaxDelayMs));
  ...
  m_streamIndex += m_MbrNeighborHelper.AssignStreams (m_adhocTxNodes, m_streamIndex);
}
...
VanetRoutingExperiment::SetupScenario ()
{
  ...
  if (m_scenario == 1)
  ...
  else if (m_scenario == 3)
    {
      ...
      m_traceFile = "/home/wu/workspace/ns-3/ns-3.26/src/wave/examples/Raleigh_Downtown50.ns2";
      ...
      if (m_loadBuildings != 0)
        {
          std::string bldgFile = "/home/wu/workspace/ns-3/ns-3.26/src/wave/examples/Raleigh_Downtown.buildings.xml";
